...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Power HAL AIDL Service for X00TD is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Power HAL AIDL Service for X00TD with Extension is starting with config: "
              << config_path;
    ...
    LOG(ERROR) << "Power HAL AIDL Service for X00TD with Extension just died.";
